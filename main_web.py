...
for i in range(1, 4):
    dataset = "web"
    checkpoint_dir = 'runs/{0}_7500/{1}/checkpoint'.format(dataset, i)
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
    time_logging_file = 'runs/{0}_7500/{1}/time.log'.format(dataset, i)
    config_logging_file = 'runs/{0}_7500/{1}/config.log'.format(dataset, i)
    # SET UP LOGGING
    ...
    # load data
    dataset = Data(sample_len=sample_len, size=7500, name=dataset)
