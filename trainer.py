...
from gan.ttsgan import TTSGenerator, TTSDiscriminator
...
class TTSGAN:
    ...
    def __init__(self,
        ...
        self.checkpoint_dir = checkpoint_dir
        self.noise_dim = noise_dim
        self.seq_len = self.real_train_dl.dataset.data_feature_shape[1]
        self.channels = self.real_train_dl.dataset.data_feature_shape[2]
        self.generator = TTSGenerator(seq_len=self.seq_len, latent_dim=self.noise_dim,
                                      channels=self.channels)
        self.discriminator = TTSDiscriminator(in_channels=self.channels, seq_length=self.seq_len, patch_size=sample_len)
        self.generator = self.generator.to(self.device)
        self.discriminator = self.discriminator.to(self.device)
        # loss
    ...
    def train(self, epochs, writer_frequency=1, saver_frequency=20):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (data_attribute, data_feature) in enumerate(self.real_train_dl):
                # data_attribute = data_attribute.to(self.device)
                data_feature = data_feature.to(self.device)
                data_feature = data_feature.view(-1, self.channels, 1, self.seq_len)
                batch_size = data_attribute.shape[0]
                # real = torch.cat((data_attribute, data_feature), dim=1)
                noise = gen_noise((batch_size, self.noise_dim)).to(self.device)
                # input_gen = torch.cat((data_attribute, noise), dim=1)
