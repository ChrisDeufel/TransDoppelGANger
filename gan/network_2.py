...
class DoppelGANgerGeneratorAttention(DoppelGANgerGenerator):
    def __init__(self, noise_dim, feature_outputs, attribute_outputs, real_attribute_mask, device, sample_len,
                 attribute_num_units=100, attribute_num_layers=3, num_heads=1, attn_dim=100, attn_mask=True,
        ...
        self.positional_encoding = PositionalEncoding(d_model=embed_dim, max_len=1000)
        # create multihead self attention
        # self.linear_q_proj = nn.Linear(in_features=embed_dim, out_features=attn_dim, bias=False)
        ...
        feature_len = len(feature_outputs)
        for i in range(len(feature_outputs) * sample_len):
            modules = [nn.Linear(embed_dim, feature_outputs[feature_counter].dim)]
            if feature_outputs[feature_counter].type_ == OutputType.DISCRETE:
                modules.append(nn.Softmax(dim=-1))
            else:
                if feature_outputs[feature_counter].normalization == Normalization.ZERO_ONE:
    ...
    def forward(self, real_attribute_noise, addi_attribute_noise, feature_input_noise):
        all_attribute, feature_gen_input = super().forward(real_attribute_noise, addi_attribute_noise,
                                                           feature_input_noise)
        feature_gen_input = self.positional_encoding(feature_gen_input)
        ...
        if self.attn_mask:
            ...
            attn_mask = None
        multihead_attn_output, _ = self.feature_multihead_attn_0(feature_gen_input, feature_gen_input,
                                                                 feature_gen_input,
                                                                 attn_mask=attn_mask)
        ...
        # multihead_attn_output = self.feature_multihead_attn(feature_gen_input)
